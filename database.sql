...
IF OBJECT_ID(N'dbo.Products', N'U') IS NOT NULL
    ...
    (
        ...
        WHERE existing_fk.parent_object_id = OBJECT_ID(N'dbo.Products')
          AND existing_fk.referenced_object_id = OBJECT_ID(N'dbo.Category')
          AND COL_NAME(
                  existing_fk.parent_object_id,
                  existing_fk.parent_column_id) = N'cate_id'
          AND COL_NAME(
                  existing_fk.referenced_object_id,
                  existing_fk.referenced_column_id) = N'cate_id'
    )
    AND NOT EXISTS
    (
        SELECT 1
...
BEGIN
    ...
    (
        ...
        createdDate DATE NULL
            CONSTRAINT DF_User_CreatedDate DEFAULT (CONVERT(DATE, GETDATE())),
        CONSTRAINT PK_User PRIMARY KEY (id),
        CONSTRAINT CK_User_RoleId CHECK (roleid BETWEEN 1 AND 3)
    );
END;
GO
...
BEGIN
    ...
    SET @CategoryTwo = ISNULL(@CategoryTwo, @CategoryOne);
    SET @CategoryThree = ISNULL(@CategoryThree, @CategoryOne);
    ...
    BEGIN
        ...
        VALUES
            (N'Áo thun nam', 20, 120000, NULL, N'Áo thun nam cơ bản.', @CategoryOne),
            (N'Áo sơ mi', 15, 180000, NULL, N'Áo sơ mi thanh lịch.', @CategoryOne),
            (N'Váy nữ', 12, 220000, NULL, N'Váy nữ thời trang.', @CategoryTwo),
            (N'Túi đeo chéo', 10, 150000, NULL, N'Túi đeo chéo tiện dụng.', @CategoryThree);
    END;
END;
GO
...
WHERE TABLE_SCHEMA = 'dbo'
  AND TABLE_NAME IN ('Category', 'Products', 'User')
...
WHERE fk.parent_object_id = OBJECT_ID(N'dbo.Products');

SELECT COUNT(*) AS category_count FROM dbo.Category;
SELECT COUNT(*) AS product_count FROM dbo.Products;
SELECT COUNT(*) AS user_count FROM dbo.[User];
GO